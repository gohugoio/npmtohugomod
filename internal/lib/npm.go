...
type Config struct {
	// Typicall set to the directory where the command is run from, but can be overridden by the user (typically for testing).
	BaseOutputDir string

	// Go module path prefix used when generating each package's go.mod, e.g.
	// "github.com/bep/npmtohugomodtestbench". When empty, Run will try to
	// derive it from BaseOutputDir's "origin" git remote.
	ModuleBase string
}
